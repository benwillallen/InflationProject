...
output:
  ...
  html_document:
    df_print: paged
subtitle: "STAT410 Final Project Report"
...
knitr::include_graphics("C:/Users/jackt/Documents/College Work/2022-2023/STAT410/Final Project/Charts/FutureForecastPlot.png")
```
...
**Note: Error values and forecast results for NNAR models are likely to differ from those presented in this report due to the in-built randomness of the model.**
...
arima_eval <- function(x_train=NULL, y_train, x_test=NULL, y_test, for_period=60, variables="FMUCOR"){
  if (is.null(x_train)){fit <- auto.arima(y_train)} else {fit <- auto.arima(y_train, xreg=as.matrix(x_train))}
  if (is.null(x_train)){fit_forecast <- forecast(fit, h=for_period)} else {fit_forecast <- forecast(fit, h=for_period, xreg=as.matrix(x_test))}
...
I devoted a significant amount of time to this project, and I really felt intrigued by the topic I chose. I wanted to explore it as thoroughly as I could with the skills and time that I had. The primary thing I learned about during this project was how to properly understand, model, and forecast time series data. I had only briefly worked with data like this, so it was a major learning experience. I had a lot of setbacks due largely to lack of background with some of the methods I implemented. I occasionally had to rework my data pipeline, and there were a lot of decisions that I switched around throughout the project. One thing I would do differently next time is to focus on getting the best data I possibly could at the start of the project rather than working with data I had easily found and just trying to fit the same the methods to better quality data. For future students, the thing that helped me the most with completing this project was getting started early. It's easy to underestimate how long a data analysis project wll take, so definitely plan for it to take more time then you might otherwise anticipate.
